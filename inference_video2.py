...
# ---------- New: Function for radial gradient background ----------
def create_radial_gradient(h, w, center_color=(1.0, 1.0, 1.0), edge_color=(0.8, 0.8, 0.8)):
    yy, xx = torch.meshgrid(torch.linspace(-1, 1, h), torch.linspace(-1, 1, w), indexing='ij')
    radius = torch.sqrt(xx**2 + yy**2).clamp(0, 1).unsqueeze(0)  # Shape: [1, H, W]
    center_color = torch.tensor(center_color).view(3, 1, 1)
    edge_color = torch.tensor(edge_color).view(3, 1, 1)
    gradient = center_color * (1 - radius) + edge_color * radius
    return gradient.unsqueeze(0)  # [1, 3, H, W]
...
parser.add_argument('--video-target-bgr-color', type=str, default=None, choices=['white', 'gradient'], help="Optional: Use 'white' or 'gradient' for target background color")
...
if args.output_format == 'video':
    h = args.video_resize[1] if args.video_resize else vid.height
    w = args.video_resize[0] if args.video_resize else vid.width
